...
///download one block
void downloadPart(UtilDownload util, DownloadTask info,
    {PartFile? partFile}) async {
  log(util.previousPart.id.toString());
  if (util.previousPart.id == 1) {
    util.download.sendPortMainThread
        .send([SendPortStatus.downloadPartIsolate, util, info, partFile]);
  } else {
    util.download.sendPortMainThread
        .send([SendPortStatus.downloadPartIsolate, util, info, partFile]);
  }
...
///one block managing
void downloadPartIsolate(UtilDownload util, DownloadTask info,
    {PartFile? partFile}) async {
  ...
  Isolate.spawn((message) async {
    ...
    receivePort.listen((message) async {
      ...
      if (util != null && info != null) {
        ...
        request.headers
            .add("Range", 'bytes=${util!.start + length}-${util!.end}');
        ...
        if ((response.contentLength - (util!.end - util!.start)).abs() < 3 &&
          ...
          savePart(response, partFile!, util!.download, info!,
