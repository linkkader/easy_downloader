...
class DownloadMonitorInside {
  ...
  void monitor() {
    ...
    if (downloadMonitor!.duration != null &&
        duration < downloadMonitor!.duration!) {
      duration = downloadMonitor!.duration!;
    }
    timer = Timer.periodic(duration, (timer) {
      if (download.parts.isNotEmpty) {
        downloadMonitor?.blockMonitor?.call(List.generate(download.parts.length,
            (index) => download.parts[index].toDownloadBlock()));
      }
      var newTotalDownloaded = 0;
      if (download.parts.isNotEmpty) {
        newTotalDownloaded = download.parts
            .map((e) => e.downloaded)
